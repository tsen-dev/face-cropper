...
camera = cv2.VideoCapture('C:/Users/Toprak/Desktop/Pexels Videos 2795168.mp4')  # Read video from specified path

while True:
    read_successful, image_bgr = camera.read()
    if not read_successful: raise RuntimeError('Image could not be read!')

    faces_rgb = face_cropper.get_normalised_faces(cv2.cvtColor(image_bgr, cv2.COLOR_BGR2RGB))
