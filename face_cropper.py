            ...
            if 0 <= face.location_data.relative_bounding_box.xmin <= 1 and 0 <= face.location_data.relative_bounding_box.ymin <= 1:
                ...
                if detected_landmarks is not None:
                    ...
                    if remove_background:
                        inflated_face_image = _get_segmented_face_image(inflated_face_image, FaceCropper._FACE_MESH, face_landmarks)
                    ...
                    if correct_roll:
                        ...
                        face_landmarks = _rotate_landmarks(face_landmarks, rotation_matrix, inflated_face_image.shape)
                        inflated_face_image = cv2.warpAffine(inflated_face_image, rotation_matrix, (inflated_face_image.shape[1], inflated_face_image.shape[0]))
                        ...
                        face_landmarks = np.ndarray.astype(np.rint(np.array([
                            ...
                            np.multiply([landmark.y for landmark in face_landmarks], inflated_face_image.shape[0])
                        ])), np.int)
